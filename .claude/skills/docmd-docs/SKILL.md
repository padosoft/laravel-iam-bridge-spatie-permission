...
name: docmd-docs
description: >
  Author and maintain the public documentation site for laravel-iam-bridge-spatie-permission, built with
  docmd and living in docs-site/. Use this skill whenever you work inside docs-site/, add or edit a docs
  page, touch navigation/plugins in docmd.config.json, change the search/build tooling, or keep the docs in
  sync with a user-facing feature change. Covers the layout, the dev/build/check commands, the container
  syntax, Lucide icons, semantic search, branding, Cloudflare Pages deploy, the page-structure standard and
  the gotchas.
---

# docmd-docs — documentation site for laravel-iam-bridge-spatie-permission

The public docs site is a [docmd](https://docs.docmd.io) static site in **`docs-site/`**. It is deployed by
the user on **Cloudflare Pages** (Git integration) at **doc.laravel-iam-bridge-spatie-permission.padosoft.com**
— do **not** add a deploy CI workflow.
...
docs-site/
  ...
  assets/favicon.svg         # teal migration mark (brand #0d9488)
...
and links GitHub + Packagist:
`© Lorenzo Padovani — [Padosoft](…) · [GitHub](…) · MIT`.
...
Sidebar groups: Get Started, Guides, Concetti & Teoria, Architettura, Best Practices, Operations, Reference.

**Accuracy first:** document what the package actually does — real classes (`SpatieScanner`,
`PermissionMapper`, `ManifestGenerator`, `ShadowGate`, `MismatchRecorder`), real commands
(`iam:spatie:scan`, `iam:spatie:manifest`), real config keys (`IAM_SPATIE_MODE`, `IAM_SPATIE_APP`,
`mismatch_log_channel`), and the real shadow semantics (`Gate::after` returns `null`; direct `hasPermissionTo`
probe; deny-overrides). A wrong doc is worse than none.
...
`.node-version` (20). The site is **doc.laravel-iam-bridge-spatie-permission.padosoft.com**.
